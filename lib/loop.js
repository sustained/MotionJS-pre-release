  ...
  Loop = (function() {
    ...
    function Loop(Game) {
      ...
      if (Motion.env === 'client') {
        ...
        this.fpsUpdate.appendTo('body');
        this.fpsRender.appendTo('body');
      } else {
        this.frameRate = noop;
      }
      this.time = Date.now();
    ...
    Loop.prototype.loop = function() {
      ...
      while (this.accum >= this.delta) {
        ...
        this.onUpdate(this.Game, this.tick, this.delta);
      }
      ...
      this.render += 1;
      return this.onRender(this.Game, this.alpha, this.context);
    };
    return Loop;
  })();
  return Loop;
