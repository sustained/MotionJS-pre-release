...
define(['entity', 'math/vector', 'geometry/polygon', 'geometry/rectangle', 'collision/aabb', 'behaviours/clickable', 'behaviours/draggable'], function(Entity, Vector, Polygon, Rectangle, AABB, BClickable, BDraggable) {
  ...
  Blob = (function() {
    ...
    Blob.prototype.input = function(input) {
      var a, b, change, hMovement, theta;
      change = 2;
      hMovement = false;
      ...
      this.theta = theta;
      this.logger.log(Math.degrees(theta));
      return;
    ...
    Blob.prototype.drawGun = function() {
      if (!this.hasAGun) {
        return false;
      }
      if (this.facingLeft) {
        canvas.rectangle(new Vector(this.body.x - 28, this.body.y), [12, 4], {
          mode: 'center',
          fill: 'orange'
        });
        return canvas.rectangle(new Vector(this.body.x - 24, this.body.y + 3), [4, 8], {
          mode: 'center',
          fill: 'orange'
        });
      } else if (this.facingRight) {
    ...
    Blob.prototype.drawCrosshairs = function(context) {
      context.translate(this.body.position.i, this.body.position.j);
      context.rotate(this.theta);
      canvas.rectangle(new Vector(0, 0), [20, 10], {
        mode: 'center',
        fill: 'purple'
      });
      context.rotate(-this.theta);
      return context.translate(-this.body.position.i, -this.body.position.j);
    };
    Blob.prototype.render = function(context) {
      ...
      this.drawMouth();
      this.drawAABB();
      return this.drawCrosshairs(context);
    };
    Blob.prototype.hasAGun = false;
    function Blob() {
