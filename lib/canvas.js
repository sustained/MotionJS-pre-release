define(['math/vector'], function(Vector) {
  ...
  Canvas = (function() {
    ...
    Canvas.prototype.lineV = function(position, direction, style) {
      return this.line(position, direction, style);
    };
    ...
    Canvas.prototype.rectangle = function(position, dimensions, style) {
      ...
      if (style.fill) {
        this.context.fillStyle = style.fill;
        this.context.fillRect(position.i.round(), position.j.round(), dimensions[0], dimensions[1]);
      }
      if (style.stroke) {
        this.context.lineWidth = style.width || 1.0;
        this.context.strokeStyle = style.stroke;
        return this.context.strokeRect(position.i.round(), position.j.round(), dimensions[0], dimensions[1]);
      }
    };
