define(['math/vector', 'geometry/circle', 'geometry/polygon'], function(Vector, Circle, Polygon) {
  /*
  ...
  SeperatingAxisTheorem = (function() {
    ...
    SeperatingAxisTheorem.test = function(a, b, statik) {
      var aIsPoly, bIsPoly;
      if (statik == null) {
        statik = true;
      }
      ...
      if (!aIsPoly && !bIsPoly) {
        return this.checkCircles(a, b, statik);
      }
      if (aIsPoly && bIsPoly) {
        return this.checkPolygons(a, b, statik);
      }
      if (!aIsPoly) {
    ...
    SeperatingAxisTheorem.checkPolygons = function(a, b, statik) {
      var axis, collision, distance, i, j, max1, max2, min1, min2, offset, pA, pB, shortestDistance, test1, test2, testNum, vertsA, vertsALen, vertsB, vertsBLen;
      i = 0;
      j = 0;
      min1 = 0;
      max1 = 0;
      ...
      /*
      			...
      			*/
      while (i < vertsALen) {
        axis = statik ? a.axes[i] : this.findNormalAxis(vertsA, i);
        pA = a.project(axis);
        pB = b.project(axis);
        if (pA[0] - pB[1] > 0 || pB[0] - pA[1] > 0) {
          return false;
        }
        distance = -(pB[1] - pA[0]);
      ...
      while (j < vertsBLen) {
        axis = this.findNormalAxis(vertsB, j);
        pA = a.project(axis);
        pB = b.project(axis);
        if (pA[0] - pB[1] > 0 || pB[0] - pA[1] > 0) {
          return false;
