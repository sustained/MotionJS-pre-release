var __hasProp = Object.prototype.hasOwnProperty, __extends = function(child, parent) {
  for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; }
  function ctor() { this.constructor = child; }
  ctor.prototype = parent.prototype;
  child.prototype = new ctor;
  child.__super__ = parent.prototype;
  return child;
};
define(['geometry/shape', 'math/vector', 'collision/aabb'], function(Shape, Vector, AABB) {
  var Polygon;
  return Polygon = (function() {
    __extends(Polygon, Shape);
    Polygon.createAABB = function(position, extents) {
      var aabb;
      if (extents == null) {
        extents = {};
      }
      extents = Motion.extend({
        t: 0,
        r: 0,
        b: 0,
        l: 0
      }, extents);
      return aabb = new this([]);
    };
    Polygon.createShape = function(sides, radius, position) {
      var angle, i, rotation, vertices;
      if (radius == null) {
        radius = 100;
      }
      if (position == null) {
        position = new Vector;
      }
      if (sides < 3) {
        return false;
      }
      angle = 0;
      rotation = Math.TAU / sides;
      vertices = [];
      i = 0;
      while (i < sides) {
        angle = (i * rotation) + ((Math.PI - rotation) * 0.5);
        vertices.push(new Vector(Math.cos(angle), Math.sin(angle)).multiply(radius));
        i++;
      }
      return new this(vertices, position, [radius, radius]);
    };
    Polygon.createRectangle = function(width, height, position) {
      var hH, hW, rect;
      if (position == null) {
        position = new Vector;
      }
      hW = width / 2;
      hH = height / 2;
      rect = new this([new Vector(-hW, -hH), new Vector(hW, -hH), new Vector(hW, hH), new Vector(-hW, hH)], position, [width, height]);
      return rect;
    };
    Polygon.createSquare = function(width, position) {
      return this.createRectangle(width, width, position);
    };
    Polygon.prototype.area = 0;
    Polygon.prototype.center = null;
    Polygon.prototype.offset = null;
    Polygon.prototype.calculateAABBExtents = function() {
      var maxX, maxY, minX, minY, v, _i, _len, _ref;
      minX = minY = Number.MAX_VALUE;
      maxX = maxY = -Number.MAX_VALUE;
      _ref = this.vertices;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        v = _ref[_i];
        if (v.i < minX) {
          minX = v.i;
        }
        if (v.i > maxX) {
          maxX = v.i;
        }
        if (v.j < minY) {
          minY = v.j;
        }
        if (v.j > maxY) {
          maxY = v.j;
        }
      }
      return {
        t: minY.abs(),
        b: maxY,
        l: minX.abs(),
        r: maxX
      };
    };
    function Polygon(_vertices, position, size) {
      this._vertices = _vertices != null ? _vertices : [];
      if (position == null) {
        position = new Vector;
      }
      if (size == null) {
        size = [0, 0];
      }
      Polygon.__super__.constructor.call(this, position);
      this.size = size;
      this._axes = [];
      this._verticesT = [];
      this.projections = [];
      this.defineAxes();
      this.defineProjections();
    }
    Polygon.get('axes', function() {
      return this._axes;
    });
    Polygon.get('vertices', function() {
      return this._vertices;
    });
    Polygon.get('verticesT', function() {
      var transformed, vert, _i, _len, _ref;
      if (this.transformed === false) {
        transformed = [];
        this.transformed = true;
        _ref = this._vertices;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          vert = _ref[_i];
          transformed.push(vert.transform(this.transform));
        }
        this._verticesT = transformed;
      }
      return this._verticesT;
    });
    Polygon.prototype.draw = function(g) {
      var vec, verts, _i, _len;
      verts = this.verticesT;
      g.beginPath();
      g.moveTo(verts[0].i, verts[0].j);
      for (_i = 0, _len = verts.length; _i < _len; _i++) {
        vec = verts[_i];
        g.lineTo(vec.i, vec.j);
      }
      g.lineTo(verts[0].i, verts[0].j);
      g.closePath();
      if (this.fill) {
        g.fillStyle = this.fill.toString();
        g.fill();
      }
      if (this.stroke) {
        g.strokeStyle = this.stroke.toString();
        return g.stroke();
      }
    };
    Polygon.prototype.defineArea = function() {
      var i, j, l, sum1, sum2;
      sum1 = sum2 = 0;
      i = j = 0;
      l = this.vertices.length;
      while (i < l) {
        j = (i + 1) % l;
        sum1 += this.vertices[i].i * this.vertices[j].j;
        sum2 += this.vertices[i].j * this.vertices[j].i;
        i++;
      }
      return this.area = 0.5 * (sum1 - sum2).abs();
    };
    Polygon.prototype.defineCenter = function() {
      var length, totalX, totalY, vector, _i, _len, _ref;
      totalX = totalY = 0;
      length = this.vertices.length;
      _ref = this.vertices;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        vector = _ref[_i];
        totalX += vector.i;
        totalY += vector.j;
      }
      return this.center = new Vector(totalX / length, totalY / length);
    };
    Polygon.prototype.collide = function(polygon) {};
    Polygon.prototype.project = function(axis) {
      var dot, i, l, max, min;
      min = max = axis.dot(this.verticesT[0]);
      i = 1;
      l = this.verticesT.length;
      while (i < l) {
        dot = axis.dot(this.verticesT[i]);
        if (dot < min) {
          min = dot;
        }
        if (dot > max) {
          max = dot;
        }
        i++;
      }
      return [min, max];
    };
    Polygon.prototype.cacheSelfProjection = function() {
      var axes, axis;
      axes = [];
      while (i < this.verticesT.length) {
        axis = this.findNormalAxis(this.verticesT, i);
        axes.push(this.project(axis));
        i++;
      }
      return axes;
    };
    /*
    		@findNormalAxis: (vertices, index) ->
    			a = vertices[index]
    			b = if index >= vertices.length - 1 then vertices[0] else vertices[index + 1]

    			new Vector( -(b.j - a.j), (b.i - a.i)).normalize()

    		*/
    Polygon.prototype.defineAxes = function() {
      var a, b, i, l, _results;
      i = 0;
      l = this.vertices.length;
      _results = [];
      while (i < l) {
        a = this.vertices[i];
        b = this.vertices[++i % l];
        _results.push(this._axes.push(Vector.subtract(b, a).rightNormal().normalize()));
      }
      return _results;
    };
    Polygon.prototype.defineProjections = function() {
      var i, l, _results;
      i = 0;
      l = this.axes.length;
      _results = [];
      while (i < l) {
        this.projections.push(this.project(this.axes[i]));
        _results.push(i++);
      }
      return _results;
    };
    Polygon.prototype.removeDuplicateAxes = function() {
      var i, j, l;
      i = j = 0;
      l = this.axes.length;
      while (i < l) {
        while (j < l) {
          if (this.axes[i].equals(this.axes[j]) || this.axes[i].equals(Vector.invert(this.axes[j]))) {
            this.axes.splice(j, 1);
          }
          j++;
        }
        i++;
      }
    };
    return Polygon;
  })();
});